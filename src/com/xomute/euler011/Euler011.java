...
public class Euler011 {
  ...
  public static int greatestProduct() {
    ...
    for (int i = 0; i < grid.length; i++) {
      for (int j = 0; j < grid[0].length; j++) {
        ...
        int product2 = 1;
        if (i < grid.length - 3) {
          product2 *= grid[i][j] * grid[i + 1][j] * grid[i + 2][j] * grid[i + 3][j];
        }

        int product3 = 1;
        if (i < grid.length - 3 && j < grid[0].length - 3) {
          product3 *= grid[i][j] * grid[i + 1][j + 1] * grid[i + 2][j + 2] * grid[i + 3][j + 3];
        }

        int product4 = 1;
        if (i < grid.length - 3 && j > 3) {
          product4 *= grid[i][j] * grid[i + 1][j - 1] * grid[i + 2][j - 2] * grid[i + 3][j - 3];
        }

        if (product1 > max) max = product1;
